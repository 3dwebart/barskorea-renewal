...
*/
	...
	for ($i = 0; $i < $cnt; $i++) {
		for ($j=0; $j < $cnt2; $j++) {
			if ($check[$i] == $chk_cnt[$j]) {
				$sql = "UPDATE bbs_config SET bo_group_id = '%s', bo_group_name = '%s', bo_name = '%s', title = '%s', skin = '%s', l_level = '%s', r_level = '%s', w_level = '%s' WHERE id = %d";
				$result = $db -> query($sql, array($bo_group_id[$j], $bo_group_name[$j], $bo_name[$j], $title[$j], $skin[$j], $l_level[$j], $r_level[$j], $w_level[$j], $chk_cnt[$j]));
			}
		}
	}
	//exit();
	...
	redirect($admin_url.'/adm_board_list.php', '수정 되었습니다.', FALSE);
?>
